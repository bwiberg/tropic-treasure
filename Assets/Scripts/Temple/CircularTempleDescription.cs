...
public class CircularTempleDescription {
	public GameObject toGameObject(GameObject templePrefab, GameObject levelPrefab, GameObject segmentPrefab) {
		var templeGameObject = GameObject.Instantiate(templePrefab) ;
		templeGameObject.name = "Circular Temple";

        var levelIndex = 0;
        foreach (var level in Levels) {
            var levelGameObject = level.toGameObject(levelIndex++, levelPrefab, segmentPrefab);
